...
import com.wj.android.wjframe.http.WJHttpException;
...
public class ImageDisplayer {
    private WJHttp kjHttp; // 使用KJHttp的线程池执行队列去加载图片
     ...
     */
    public ImageDisplayer(WJHttp http, BitmapConfig bitmapConfig) {
        kjHttp = http;
        ...
        return new ImageRequest(requestUrl, maxWidth, maxHeight,
                new HttpCallBack() {
                    ...
                    public void onFailure(int errorNo, String strMsg) {
                        ...
                        onGetImageError(requestUrl, new WJHttpException(strMsg));
                    }
                });
    }
     ...
     * @param error 失败原因
     */
    protected void onGetImageError(String url, WJHttpException error) {
        // 从正在请求的列表中移除这个已完成的请求
        ImageRequestEven request = mRequestsMap.remove(url);
        if (request != null) {
    ...
    private class ImageRequestEven {
        ...
        private WJHttpException mError;
        ...
        public void setError(WJHttpException error) {
            mError = error;
        }

        public WJHttpException getError() {
            return mError;
        }
