...
public final class SystemTool {
    ...
    public static boolean is3G(Context cxt) {
        ...
        // wifi的状态：ConnectivityManager.TYPE_WIFI
        // 3G的状态：ConnectivityManager.TYPE_MOBILE
        NetworkInfo.State state = cm.getNetworkInfo(ConnectivityManager.TYPE_MOBILE)
                .getState();
    ...
    public static boolean checkOnlyWifi(Context context) {
        if (PreferenceHelper.readBoolean(context, WJConfig.SETTING_FILE,
                WJConfig.ONLY_WIFI)) {
    ...
    public static int gc(Context cxt) {
        ...
        WJLoger.debug("清理了" + (getDeviceUsableMemory(cxt) - i) + "M内存");
        return count;
    }
