...
public class NetworkDispatcher extends Thread {
    ...
    public void run() {
        ...
        while (true) {
            ...
            } catch (WJHttpException volleyError) {
                parseAndDeliverNetworkError(request, volleyError);
            } catch (Exception e) {
                WJLoger.debug("Unhandled exception %s", e.getMessage());
                mDelivery.postError(request, new WJHttpException(e));
            }
        }
    }

    private void parseAndDeliverNetworkError(Request<?> request,
                                             WJHttpException error) {
